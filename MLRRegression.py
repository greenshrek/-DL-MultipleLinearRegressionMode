...
def gradient_descent(bias, coefficients, alpha, X, Y, max_iter):
    ...
    for num in range(0, max_iter):
        ...
        #linefunc = (X * coefficients + bias)
        linefunc = np.sum((X * coefficients + bias), axis = 1)
        print(linefunc)

        hx = 1/(1 + np.exp(-linefunc))
        
        dCoeff = np.sum(np.divide(np.dot(X.transpose(), hx-Y), 2*(X.shape[0])))

        dBias = np.sum(np.divide((hx-Y), 2*(X.shape[0])))
        
        coefficients = coefficients - alpha * dCoeff
...
def main():
    ...
    df = pd.read_csv("boston-tiny.csv")
    df = df.dropna()
    print (df.shape)
